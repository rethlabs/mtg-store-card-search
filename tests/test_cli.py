...
class CliTests(unittest.TestCase):
    def test_repeatable_city_and_card_arguments(self):
        args = build_parser().parse_args(
            ["find", "--city", "Houston", "--city", "Dallas", "--card", "Sol Ring"]
        )
        self.assertEqual(args.city, ["Houston", "Dallas"])
        self.assertEqual(args.card, ["Sol Ring"])

    def test_request_file_loads_printing_filters(self):
        payload = {
            "areas": [{"city": "Houston", "state": "TX"}],
        ...
        self.assertEqual(areas[0].label, "Houston, TX")
